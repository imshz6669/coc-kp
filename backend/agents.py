...
def _extract_from_narrative(text: str) -> Dict[str, str]:
    ...
    # scene：匹配常见 COC 地点模式（严格匹配，宁缺毋滥）
    scene = ""
    place_patterns = [
        # 精确的地点词（优先级最高）
        (r'(?:废弃的?|破败的?|阴暗的?|潮湿的?)?(?:城隍庙|祠堂|茶馆|码头|客栈|酒馆|书房|阁楼|地窖|卧室|客厅|厨房|走廊|温室|墓地)', ''),
        (r'(?:沈家|温斯洛普)?(?:祖宅|庄园)(?:的?)(?:大门|前院|后院|大厅)', ''),
        (r'(?:地下|天然|一间|一个)?(?:密室|石室|暗室|洞穴)', ''),
        (r'(?:禁地|镇外)?(?:的?)(?:石碑|深潭)', ''),
        (r'后院.*?(?:黑棺|木棺|墓地)', ''),
        (r'枯井|前院.*井', ''),
        (r'(?:地下|一楼|二楼|三楼)(?:室|层|间)', ''),
        # 英文地点词（古宅疑云类西式剧本）
        (r'(?:Winslow|温斯洛普)\s*(?:Manor|庄园|Hall|大厅|Library|书房|Attic|阁楼|Basement|地下室|Cave|洞穴)', ''),
    ]
    for pattern, _ in place_patterns:
        m = re.search(pattern, text)
        if m:
            scene = m.group(0).strip()[:8]  # 限制8字
            # 清理掉叙事碎片（如果匹配结果包含奇怪的前缀）
            if any(bad in scene for bad in ['每次', '那个', '站在', '方的', '到了', '说着']):
                scene = ""  # 拒绝质量差的匹配
                continue
            break

    # 补充：如果仍未匹配，从叙事中提取 "在XX" 之后的地点
    if not scene:
        loc_match = re.search(r'(?:在|到了|进入了?|来到了?|回到了?)([\w一-鿿]{2,6})(?:[，。,\s]|$)', text)
        if loc_match:
            candidate = loc_match.group(1).strip()
            # 拒绝明显不是地点的词
            bad_words = ['你', '我', '他', '她', '它', '这', '那', '一', '是', '的', '了', '着', '过',
                        '不', '很', '都', '也', '就', '要', '会', '能', '可', '被', '把', '向', '从']
            if candidate not in bad_words and len(candidate) >= 2:
                scene = candidate[:8]
